...
This must be called from inside [`RDDIP.parameterize`](@ref).
...
This must be called outside [`RDDIP.parameterize`](@ref).
...
"""
    set_trade_off_weight(model::PolicyGraph, weight::Float64)
...
!!! warning
    This function is experimental! It may change in any future release.
"""
function set_trade_off_weight(model::PolicyGraph, weight::Float64)
    @assert 0 <= weight <= 1
 ...
 * `kwargs` are passed to [`train`](@ref) when solving the scalarized
   problems.
...
function train_biobjective(
    ...
    solution_limit::Int,
    include_timing::Bool = false,
    log_file_prefix::String = "RDDIP",
    stopping_rules::Function = weight -> AbstractStoppingRule[],
    kwargs...,
)
    start_time = time()
