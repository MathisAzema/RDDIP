function benders_callback_AVAR(instance, options;silent=true, α=0, force=1, S::Int64, batch=1, gap=0.05, timelimit=10)
    """
    Solve the two stage risk-averse SUC with Benders' Decomposition
    """
    T= instance.TimeHorizon
    N=instance.N
    thermal_units=values(instance.Thermalunits)

    master_pb=master_AVAR_problem(instance, silent=silent, S=S, α=α)
    oracle_pb=oracle_SP_problem(instance)
    
    if options._add_optimality_cuts==_add_optimality_cuts_extended_exact
        for unit in thermal_units
            for (a,b) in unit.intervals
                instance.model_Q_jab[unit.name,a,b]=initialize_model_Q_jab(T, unit, a, b, optimizer)
            end
        end
    end
    ...
    function my_callback_function(cb_data, cb_where::Cint)
        ...
        if cb_where==GRB_CB_MIPSOL
            ...
            second_stage_cost_ub, thermal_cost_scenario = options.add_cut(cb_data, options, master_pb, oracle_pb, instance, Time_subproblem, solution_x, solution_gamma; force=force, S=S, batch=batch, gap=gap, α=α)
            if second_stage_cost_ub<= 0.999999*UB
                ...
                if k>=10 #Force to the new best solution after 10 iterations
                    vars= vcat([master_pb[:is_on][i,t] for i in 1:N for t in 0:T], [master_pb[:start_up][i,t] for i in 1:N for t in 1:T], [master_pb[:start_down][i,t] for i in 1:N for t in 1:T], [master_pb[:gamma][i, [a,b]] for i in 1:N for (a,b) in instance.Thermalunits[i].intervals],
                        [master_pb[:thermal_fixed_cost], master_pb[:thermal_cost]], [master_pb[:thermal_fuel_cost][s] for s in 1:S], [master_pb[:thermal_fuel_cost_pos][s] for s in 1:S], [master_pb[:z_AVAR]])
                    ...
                    vals= vcat([solution_is_on[i,t+1] for i in 1:N for t in 0:T], [solution_start_up[i,t] for i in 1:N for t in 1:T], [solution_start_down[i,t] for i in 1:N for t in 1:T], [solution_gamma[i,a,b] for i in 1:N for (a,b) in instance.Thermalunits[i].intervals],
                        [callback_value.(cb_data, master_pb[:thermal_fixed_cost]), sum(max(0, thermal_cost_scenario[s]-z_AVAR_val) for s in 1:S)/S], [thermal_cost_scenario[s] for s in 1:S], [max(0, thermal_cost_scenario[s]-z_AVAR_val) for s in 1:S], z_AVAR_val)
    ...
    if feasibleSolutionFound
        ...
        return instance.name, computation_time, objective_value(master_pb), objective_bound(master_pb), current_solution, S,batch, Time_subproblem, gap, force, α
    else
        return nothing
    end
end