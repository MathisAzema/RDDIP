...
function thermal_unit_capacity_constraints_scenarios2(model, instance, S)
    """
    Define second-stage constraints in the extensive formulation
    """
    power=model[:power]
    power_real=model[:power_real]
    is_on=model[:is_on]
    start_up=model[:start_up]
    start_down=model[:start_down]
    thermal_fuel_cost=model[:thermal_fuel_cost]
    ...
    @constraint(model,  [unit in thermal_units, t in 0:T, s in 1:S], power_real[unit.name, t, s]>=unit.MinPower*is_on[unit.name, t, s])
    @constraint(model,  [unit in thermal_units, t in 0:T, s in 1:S], power_real[unit.name, t, s]<=unit.MaxPower*is_on[unit.name, t, s])
    @constraint(model,  [unit in thermal_units, t in 0:T, s in 1:S], power[unit.name, t, s]>=unit.MinPower*is_on[unit.name, t, s])
    @constraint(model,  [unit in thermal_units, t in 0:T, s in 1:S], power[unit.name, t, s]<=unit.MaxPower*is_on[unit.name, t, s])
    @constraint(model,  [unit in thermal_units, s in 1:S; unit.InitialPower!=nothing], power_real[unit.name, 0, s]==unit.InitialPower)

    @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S], power_real[unit.name, t, s]-power[unit.name, t-1, s]<=(-unit.DeltaRampUp)*start_up[unit.name, t, s]+(unit.MinPower+unit.DeltaRampUp)*is_on[unit.name, t, s]-(unit.MinPower)*is_on[unit.name, t-1, s])
    @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S], power[unit.name, t-1, s]-power_real[unit.name, t, s]<=(-unit.DeltaRampDown)*start_down[unit.name, t, s]+(unit.MinPower+unit.DeltaRampDown)*is_on[unit.name, t-1, s]-(unit.MinPower)*is_on[unit.name, t, s])

    @constraint(model, [s in 1:S], thermal_fuel_cost[s]>=sum(unit.LinearTerm*power_real[unit.name, t, s] for unit in thermal_units for t in 1:T)+sum(SHEDDING_COST*power_shedding[b,t,s]+CURTAILEMENT_COST*power_curtailement[b,t,s] for b in Buses for t in 1:T))
end

function thermal_unit_commit_constraints2(model, instance, S)
    """
    Define first-stage constraints in the 3-bin formulation
    """
    is_on=model[:is_on]
    ...
    @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S], is_on[unit.name, t, s]-is_on[unit.name, t-1, s]==start_up[unit.name, t, s]-start_down[unit.name, t, s])
    @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S], start_up[unit.name, t, s]<=is_on[unit.name, t, s])
    @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S], start_down[unit.name, t, s]<=1-is_on[unit.name, t, s])

    # @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S; unit.InitUpDownTime!=nothing], sum(start_up[unit.name, τ, s] for τ in max(1, t-unit.MinUpTime+1):t)+1*(t<unit.MinUpTime-unit.InitUpDownTime+1)*(unit.InitUpDownTime>0) <= is_on[unit.name, t, s])
    # @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S; unit.InitUpDownTime!=nothing], sum(start_down[unit.name, τ, s] for τ in max(1, t-unit.MinDownTime+1):t)+1*(t<unit.MinDownTime+unit.InitUpDownTime+1)*(unit.InitUpDownTime<0) <= 1-is_on[unit.name, t, s])
    # @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S; unit.InitUpDownTime==nothing], sum(start_up[unit.name, τ, s] for τ in max(1, t-unit.MinUpTime+1):t) <= is_on[unit.name, t, s])
    # @constraint(model,  [unit in thermal_units, t in 1:T, s in 1:S; unit.InitUpDownTime==nothing], sum(start_down[unit.name, τ, s] for τ in max(1, t-unit.MinDownTime+1):t) <= 1-is_on[unit.name, t, s])

    @constraint(model,  [unit in thermal_units, s in 1:S; unit.InitUpDownTime!=nothing], is_on[unit.name, 0, s]==(unit.InitUpDownTime>=0))

    @constraint(model,  [unit in thermal_units, s in 1:S; unit.InitUpDownTime==nothing], is_on[unit.name, 0, s]==0)

    @constraint(model, [s in 1:S], thermal_fixed_cost[s]>=sum(unit.ConstTerm*is_on[unit.name, t, s]+unit.StartUpCost*start_up[unit.name, t, s]+unit.StartDownCost*start_down[unit.name, t, s] for unit in thermal_units for t in 1:T))
    ...
    for unit in thermal_units
        ...
        for t in 0:limit
            @constraint(model, [s in 1:S], is_on[unit.name, t, s]==1)
        end
    end
end