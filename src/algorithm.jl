...
# risk_measure = RDDIP.Expectation(),
...
# Internal struct: storage for RDDIP options and cached data. Users shouldn't
...
struct Options{T}
    ...
    function Options(
        ...
        stopping_rules::Vector{AbstractStoppingRule} = AbstractStoppingRule[],
...
The `throw_error` is an argument used internally by RDDIP.jl. If set, an error
will be thrown.

## Example

```julia
RDDIP.write_subproblem_to_file(model[1], "subproblem_1.lp")
```
"""
function write_subproblem_to_file(
    ...
    if throw_error
        error(
            ...
            "See https://odow.github.io/RDDIP.jl/stable/tutorial/warnings/ for more information.",
        )
    end
    return
end
...
RDDIP.set_numerical_difficulty_callback(model, callback)
```
"""
...
# Internal function: perform a backward pass of the RDDIP algorithm along the
...
"""
    RDDIP.train(model::PolicyGraph; kwargs...)
...
## Keyword arguments
 ...
 - `stoping_rules`: a vector of [`RDDIP.AbstractStoppingRule`](@ref)s. Defaults
 ...
 - `log_file::String`: filepath at which to write a log of the training
   progress. Defaults to `RDDIP.log`.
 ...
 - `refine_at_similar_nodes::Bool`: if RDDIP can detect that two nodes have the
 ...
 - `cut_type`: choose between `RDDIP.SINGLE_CUT` and `RDDIP.MULTI_CUT` versions of
   RDDIP.
...
function train(
    ...
    # something like stopping_rules = [RDDIP.IterationLimit(100)], the vector
...
## Keyword arguments
 ...
 - `duality_handler`: the [`RDDIP.AbstractDualityHandler`](@ref) used to compute
...
```julia
simulation_results = RDDIP.simulate(model, 2;
    custom_recorders = Dict{Symbol, Function}(
...
```julia
rule = RDDIP.DecisionRule(model; node = 1)
```
"""
