...
mutable struct InnerConvexApproximation
    ...
    sampled_states::Vector{RDDIP.SampledState}
...
"""
    InnerBellmanFunction(Lipschitz_constant;
        ...
        vertex_type::RDDIP.CutType = RDDIP.MULTI_CUT,
    )
"""
function InnerBellmanFunction(
    ...
    return RDDIP.InstanceFactory{InnerBellmanFunction}(;
...
function RDDIP.bellman_term(bellman_function::InnerBellmanFunction)
...
# See [`to_nodal_form`] in RDDIP.jl
function to_node(node::RDDIP.Node{T}, element) where {T}
    return element
end

function to_node(node::RDDIP.Node{T}, builder::Function) where {T}
    return builder(node.index)
end

function to_node(node::RDDIP.Node{T}, dict::Dict{T,V}) where {T,V}
    return dict[node.index]
end

function RDDIP.initialize_bellman_function(
    factory::RDDIP.InstanceFactory{InnerBellmanFunction},
    ...
    deletion_minimum,
    vertex_type,
    Lipschitz_constant = -Inf, Inf, 0, RDDIP.SINGLE_CUT, 0.0
    ...
    RDDIP._add_initial_bounds(node.objective_state, Θᴳ)
...
function RDDIP.refine_bellman_function(
    model::RDDIP.PolicyGraph{T},
    node::RDDIP.Node{T},
    bellman_function::InnerBellmanFunction,
    risk_measure::RDDIP.AbstractRiskMeasure,
...
function _refine_inner_bellman_function_no_lock(
    ...
    offset = RDDIP.adjust_probability(
        risk_measure,
    ...
    if bellman_function.cut_type == RDDIP.SINGLE_CUT
        ...
        @assert bellman_function.cut_type == RDDIP.MULTI_CUT
        error("Multi-vertices not yet implemented.")
        # RDDIP._add_locals_if_necessary(
        #     node,
        #     bellman_function,
...
function model_type(model::RDDIP.PolicyGraph{T}) where {T}
    return T
end

"""
    _validate_linear_graph(graph::RDDIP.Graph)
...
the main RDDIP module and is meant to be used in conjunction with it while
the Inner module does not support other graphs.

"""
function _validate_linear_graph(graph::RDDIP.Graph)
...
(See [`RDDIP.Graph`](@ref) for details.)
...
function InnerPolicyGraph(
    ...
    for (node_index, children) in graph.nodes
        ...
        subproblem = RDDIP.construct_subproblem(optimizer, direct_mode)
        node = RDDIP.Node(
            node_index,
            subproblem,
            RDDIP.Noise{T}[],
        ...
        if length(node.noise_terms) == 0
            push!(node.noise_terms, RDDIP.Noise(nothing, 1.0))
        end
    ...
    for (node_index, children) in graph.nodes
        ...
        for (child, probability) in children
            push!(node.children, RDDIP.Noise(child, probability))
        end
        # Intialize the bellman function. (See note in creation of Node above.)
        node.bellman_function = RDDIP.initialize_bellman_function(
            bellman_function,
            policy_graph,
            node,
        )
    ...
    for (child, probability) in graph.nodes[graph.root_node]
        push!(policy_graph.root_children, RDDIP.Noise(child, probability))
    ...
    domain = RDDIP._get_incoming_domain(policy_graph)
...
"""
    dp_vertices_from_visited_states(
        inner_model::RDDIP.PolicyGraph
        outer_model::RDDIP.PolicyGraph;
        optimizer = nothing,
        risk_measures::RDDIP.AbstractRiskMeasure = RDDIP.Expectation(),
        print_level::Int = 0,
    )
...
## Keyword arguments
 ...
 - `risk_measures::RDDIP.AbstractRiskMeasure` risk measures
    for the problem.
...
function dp_vertices_from_visited_states(
    ...
    opts =
        RDDIP.Options(inner_model, outer_model.initial_root_state; risk_measures)
    ...
    for node_index in sort(collect(keys(outer_model.nodes)); rev = true)[2:end]
        dt = @elapsed begin
            ...
            for sampled_state in fw_samples
                ...
                items = RDDIP.BackwardPassItems(T, RDDIP.Noise)
                RDDIP.solve_all_children(
                    inner_model,
                    node,
                    items,
                    1.0,
...
"""
    inner_dp(
        ...
        bellman_function,
        risk_measures::RDDIP.AbstractRiskMeasure,
        print_level::Int = 1,
    )
...
## Keyword arguments
 ...
 - `risk_measure::RDDIP.AbstractRiskMeasure` risk measures for the problem.
...
function inner_dp(
    ...
    pb_inner = RDDIP.LinearPolicyGraph(
        build;
        stages,
        sense,
        optimizer,
    ...
    ub = RDDIP.calculate_bound(pb_inner; risk_measure)
    if print_level > 0
...
See also [`RDDIP.read_vertices_from_file`](@ref) and [`RDDIP.write_cuts_to_file`](@ref).
"""
...
function read_vertices_from_file(
    model::RDDIP.PolicyGraph{T},
    filename::String;
    node_name_parser::Function = RDDIP._node_name_parser,
    vertex_name_parser::Function = _vertex_name_parser,
