...
Run RDDIP in asynchronous mode workers with pid's `slave_pids`.
...
Run RDDIP in multi-threaded mode.
...
!!! danger
    This plug-in is experimental, and parts of RDDIP.jl may not be threadsafe. If
    you encounter any problems or crashes, please open a GitHub issue.

## Example

```julia
RDDIP.train(model; parallel_scheme = RDDIP.Threaded())
RDDIP.simulate(model; parallel_scheme = RDDIP.Threaded())
```
"""
