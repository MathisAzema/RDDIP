...
struct RobustMonteCarlo <: AbstractSamplingScheme
end
...
a vector of new [`RDDIP.Noise`](@ref) terms for the children of that node, and
a vector of new [`RDDIP.Noise`](@ref) terms for the stagewise-independent
noise.
...
[`RDDIP.Noise`](@ref) for the children of the root node.
...
If a node is deterministic, pass `[RDDIP.Noise(nothing, 1.0)]` as the vector of
noise terms.
...
sampler = OutOfSampleMonteCarlo(graph) do node
    if node == 0
        return [RDDIP.Noise(1, 1.0)]
    else
        noise_terms = [RDDIP.Noise(node, 0.3), RDDIP.Noise(node + 1, 0.7)]
        children = node < T ? [RDDIP.Noise(node + 1, 0.9)] : RDDIP.Noise{Int}[]
...
sampler = OutOfSampleMonteCarlo(graph, use_insample_transition=true) do node
    return [RDDIP.Noise(node, 0.3), RDDIP.Noise(node + 1, 0.7)]
end
```
"""
...
function sample_noise(noise_terms::Vector{<:Noise})
    ...
    return error(
        "Internal RDDIP error: unable to sample noise from $(noise_terms)",
    )
end
...
function sample_scenario(
    ...
    return error(
        "Internal RDDIP error: something went wrong sampling a scenario.",
    )
end
...
The sample space for [`RDDIP.parameterize`](@ref) must be a tuple with 1 or 2
...
julia> model = RDDIP.PolicyGraph(
           RDDIP.MarkovianGraph(simulator; budget = 20, scenarios = 100);
           sense = :Max,
           upper_bound = 12,
           optimizer = HiGHS.Optimizer,
       ) do sp, node
           t, markov_state = node
           @variable(sp, x >= 0, RDDIP.State, initial_value = 1)
           @variable(sp, u >= 0)
           ...
           RDDIP.parameterize(sp, Ω) do (markov_state, ω)
...
julia> RDDIP.train(
           ...
           print_level = 0,
           iteration_limit = 10,
           sampling_scheme = RDDIP.SimulatorSamplingScheme(simulator),
       )
