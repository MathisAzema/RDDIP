...
function _deprecate_integrality_handler()
    return error(
        """
RDDIP.jl v0.4.0 introduced a number of breaking changes in how we deal with
 ...
 * Instead of passing the argument to `PolicyGraph`, you now pass it to
   `train`, e.g., `RDDIP.train(model; duality_handler = RDDIP.LagrangianDuality())`
 ...
 * `optimizer`: if  specified, RDDIP.jl will call
...
julia> duality_handler = RDDIP.ContinuousConicDuality(Ipopt.Optimizer)
...
julia> model = RDDIP.LinearPolicyGraph(;
           ...
           @variable(sp, x, RDDIP.State, Int, initial_value = 0)
...
julia> RDDIP.train(
           model;
           duality_handler = RDDIP.ContinuousConicDuality(),
           print_level = 0,
       )
```
## Theory
...
struct ContinuousConicDuality{O} <: AbstractDualityHandler
    ...
    function ContinuousConicDuality(optimizer = nothing)
        return new{typeof(optimizer)}(optimizer, "ContinuousConicDuality")
    end
end
...
function get_dual_solution(node::Node, ::ContinuousConicDuality)
    ...
    for (name, uncertainty) in node.uncertainties
        μ[name] = dual_sign * JuMP.dual(JuMP.FixRef(uncertainty.var))
    end
    return objective_bound(node.subproblem), λ, μ
end
...
keyword argument of [`RDDIP.train`](@ref):

```jldoctest
julia> import RDDIP, HiGHS, Ipopt

julia> duality_handler = RDDIP.LagrangianDuality(Ipopt.Optimizer)
RDDIP.LagrangianDuality{DataType}(RDDIP.LocalImprovementSearch.BFGS(100), Ipopt.Optimizer)
...
julia> RDDIP.train(
           model;
           duality_handler = RDDIP.LagrangianDuality(),
           print_level = 0,
       )
```
...
mutable struct LagrangianDuality{O} <: AbstractDualityHandler
    ...
    function LagrangianDuality(
        ...
        return new{typeof(optimizer)}(method, optimizer, "LagrangianDuality")
    end
end
...
function _solve_primal_problem_lower(
    node::Node,
    model::JuMP.Model,
    λ::Vector{Float64},
    h_expr::Vector{GenericAffExpr{Float64,VariableRef}},
...
function _solve_primal_problem_upper(
    ...
    h_expr::Vector{GenericAffExpr{Float64,VariableRef}},
    μ::Vector{Float64},
    g_expr::Vector{GenericAffExpr{Float64,VariableRef}},
)
    primal_obj = JuMP.objective_function(model)

    JuMP.set_objective_function(
        model,
        @expression(model, primal_obj - λ' * h_expr - μ' * g_expr),
    )
...
function _solve_lagrangian_problem(
    ...
    if JuMP.termination_status(model) != MOI.OPTIMAL && JuMP.termination_status(model) != MOI.DUAL_INFEASIBLE
        ...
        println((node.index, length(node.lagrangian_lower.cuts)))
        # end
    ...
    μ = Dict{Symbol,Float64}()
    for (key, uncertainty) in node.uncertainties
        μ[key] = value(node.lagrangian_lower.dual_variables_uncertainty[key])
    end
...
function _refine_lagrangian_model(
    ...
    if type == 0
        cstr = @constraint(node.lagrangian_lower.model,
            node.lagrangian_lower.theta <= intercept - sum(
                ...
                node.lagrangian_lower.dual_variables_uncertainty[k] * incoming_uncertainty_values[k] for
                (k, uncertainty) in node.uncertainties)
        )
    ...
    elseif type == 1
        ...
        push!(node.lagrangian_upper.cuts, Cut2(intercept, cost_to_go_value, cstr, outgoing_state_values))
    end
...
function get_dual_solution(node::Node, handler::LagrangianConicDuality)
    ...
    for (i, (key, uncertainty)) in enumerate(node.uncertainties)
        ...
        # g_expr[i] = @expression(node.subproblem, uncertainty.var - ξ[i])
...
julia> RDDIP.train(model; duality_handler = duality_handler, print_level = 0)
```
...
mutable struct BanditDuality <: AbstractDualityHandler
    ...
    function BanditDuality(args::AbstractDualityHandler...)
        return new(_BanditArm[_BanditArm(arg, Float64[]) for arg in args], 1, 1, "BanditDuality")
    end
end
...
julia> RDDIP.train(
           model;
           duality_handler = RDDIP.FixedDiscreteDuality(),
           print_level = 0,
       )
```
"""
mutable struct FixedDiscreteDuality{O} <: AbstractDualityHandler
    ...
    function FixedDiscreteDuality(optimizer = nothing)
        return new{typeof(optimizer)}(optimizer, "FixedDiscreteDuality")
    end
end
