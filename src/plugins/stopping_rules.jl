    ...
    function Statistical(;
        ...
        if !disable_warning
            @warn(
                ...
                "RDDIP.Statistical` into the REPL to read the docstring.\n\n" *
                "If you understand what you are doing, you can disable this " *
                "warning with `RDDIP.Statistical(; disable_warning = true)`",
            )
        end
...
By default, `replications` and `period` are `-1`, and RDDIP.jl will guess good
...
```julia
RDDIP.train(model; stopping_rules = [SimulationStoppingRule()])
```
"""
...
```julia
RDDIP.train(model; stopping_rules = [FirstStageStoppingRule()])
```
"""
