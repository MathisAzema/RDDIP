...
julia> RDDIP.adjust_probability(
           RDDIP.Expectation(),
           risk_adjusted_probability,
           [0.1, 0.2, 0.3, 0.4],  # nominal_probability,
           RDDIP.Noise.([1, 2, 3, 4], [0.1, 0.2, 0.3, 0.4]),  # noise_supports,
...
```jldoctest
julia> RDDIP.ConvexCombination(
           (0.5, RDDIP.Expectation()),
           (0.5, RDDIP.AVaR(0.25))
       )
A convex combination of 0.5 * RDDIP.Expectation() + 0.5 * RDDIP.AVaR(0.25)
```
...
```jldoctest
julia> 0.5 * RDDIP.Expectation() + 0.5 * RDDIP.AVaR(0.5)
A convex combination of 0.5 * RDDIP.Expectation() + 0.5 * RDDIP.AVaR(0.5)
```
"""
...
```jldoctest
julia> RDDIP.EAVaR(; lambda = 1.0, beta = 1.0)
A convex combination of 1.0 * RDDIP.Expectation() + 0.0 * RDDIP.AVaR(1.0)

julia> RDDIP.EAVaR(; lambda = 0.0, beta = 1.0)
A convex combination of 0.0 * RDDIP.Expectation() + 1.0 * RDDIP.AVaR(1.0)

julia> RDDIP.EAVaR(; lambda = 0.5, beta = 0.5)
A convex combination of 0.5 * RDDIP.Expectation() + 0.5 * RDDIP.AVaR(0.5)
```
"""
...
The distributionally robust RDDIP risk measure of
Philpott, A., de Matos, V., Kapelevich, L. Distributionally robust RDDIP.
...
julia> RDDIP.adjust_probability(
           ...
           RDDIP.Noise.([1, 2, 3, 4], [0.25, 0.25, 0.25, 0.25]),  # noise_supports,
           [5.0, 4.0, 6.0, 2.0],  # cost_realizations,
           true,                  # is_minimization
       )
0.0
...
```julia
wasserstein_norm(x::RDDIP.Noise, y::RDDIP.Noise)::Float64
```
...
julia> wasserstein_norm(x::RDDIP.Noise, y::RDDIP.Noise) = abs(x.term - y.term);

julia> RDDIP.adjust_probability(
           RDDIP.Wasserstein(wasserstein_norm, HiGHS.Optimizer; alpha = 0.5),
           risk_adjusted_probability,
           [0.1, 0.2, 0.3, 0.4],  # nominal_probability,
           RDDIP.Noise.([1.0, 2.0, 3.0, 4.0], [0.1, 0.2, 0.3, 0.4]),  # noise_supports,
           [5.0, 4.0, 6.0, 2.0],  # cost_realizations,
           true,                  # is_minimization
       )
0.0
...
Base.show(io::IO, measure::Wasserstein) = print(io, "RDDIP.Wasserstein")
